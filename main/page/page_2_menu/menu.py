...
from settings import APP_NAME, VERSION , MAJ_DB_CONFIG, POSTGRESQL_CONFIG
...
class MenuWindow(QWidget):
    ...
    def list_bdd(self):
        ...
        if self.db_type == "PostgreSQL":
            ...
            connection_bdd, erreur = connect_to_sql_database(DB_CONFIG)
    ...
    def connection_bdd(self):
        ...
        if self.db_type == "PostgreSQL":
            ...
            DB_CONFIG = MAJ_DB_CONFIG(dbname=selected_bdd)
            connection_bdd, erreur = connect_to_database(DB_CONFIG)

        if connection_bdd is None or not connection_bdd:
            self.label_bdd.setText(f"Erreur de connexion à la base de données : {erreur}")
            return

        self.label_bdd.setText(f"Connecté à la base de données : {selected_bdd}")
