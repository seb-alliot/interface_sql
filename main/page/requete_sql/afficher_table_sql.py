...
from main.utils.module.maria_db import connect_to_maria_database, MARIA_DB_CONFIG, voir_base_maria, voir_table_maria
...
class Afficher_Table_SQL_Window(QWidget):
    ...
    def afficher_contenu_table(self):
        ...
        if choix == "MongoDB":
            try:
                if self.connection_mongo is None:
                    self.message_label.setText(f"Connexion échouée : MongoDB non connecté.")
                    return

                contenu = voir_contenu_collection_mongo(self.connection_mongo, self.choix_bdd, self.table_name)
        ...
        if choix in db_configs:
            try:
                config = db_configs[choix]["config_bdd"](dbname=self.choix_bdd)
                connector = db_configs[choix]["connector"]
                connection, erreur = connector(config)
                ...
                if 'connection' in locals() and 'curseur' in locals():
                    Close(connection, curseur)
        else:
